...
## smart_write_file  ← prefer this over write_file for existing files

**When to use:** Any time you are about to write to a path that might already exist.

**How:**
- `path`: target path (same rules as write_file).
- `content`: full new content.
- `force_overwrite` (optional, default `false`): set `true` to bypass the guard and replace the whole file.

**Behavior:**
- File **does not exist** → identical to `write_file`. Returns `Written and verified: ...`.
- File **exists + force_overwrite=false** → returns a `[no_overwrite guard]` message with the artifact ID and section IDs. Use `edit_artifact_section` to update just the section you need.
- File **exists + force_overwrite=true** → replaces the whole file (same as write_file), logs the overwrite.

**Why:** prevents the common mistake of obliterating prior work when only a small section needs changing.

---

## rollback_artifact

**When to use:** After an edit went wrong and you need to restore the file to a prior state.

**How:**
- `identifier`: artifact ID, title, or path substring.
- `steps` (optional, default 1): how many edits back to restore.

**Returns:** success message with restored byte count and timestamp of the recovered state, or an error if no snapshots exist.

**Note:** snapshots are recorded automatically every time `edit_artifact_section` or `smart_write_file` modifies an existing file. Plain `write_file` calls also create snapshots via `record_artifact`.

---

## write_file

**When to use:** Create a brand-new file, or replace a file whose entire content you have already confirmed is correct. For partial updates to existing files, prefer `smart_write_file` → `edit_artifact_section`.
