...
@pytest.mark.asyncio
async def test_run_tool_exception_handling(monkeypatch):
    from src.agent.core import AssistiveAgent
    agent = AssistiveAgent(user_id="default")
    
    def mock_list_processes(*args, **kwargs):
        raise RuntimeError("Unexpected process list error")
        
    monkeypatch.setattr("src.tools.system.list_processes", mock_list_processes)
    
    result = await agent._run_tool("list_processes", {})
    assert "unexpected exception executing tool" in result
    assert "Unexpected process list error" in result
