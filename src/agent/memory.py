...
class MemoryStore:
    ...
    def add_short_term(self, content: str, **metadata: Any) -> None:
        ...
        full_metadata = {**getattr(self, "current_turn_metadata", {}), **metadata}
        self.sessions.touch(self.session_id)
        self.episodic.insert(
            session_id=self.session_id,
            role=_infer_role(content),
            content=content,
            source=full_metadata.get("source", "agent"),
    ...
    def get_context_for_agent(self, max_immediate: int = 5, max_short: int = 20) -> str:
        ...
        if getattr(self, "current_turn_metadata", {}).get("source_device") == "workshop":
            parts.append(
                "## Workshop Context Alert\n"
                "CRITICAL: You are currently responding to Travis from inside the DEVELOPMENT WORKSHOP CODE EDITOR (Cursor Clone), NOT the normal chat UI. "
                "Travis is actively coding. Focus your response on the active workspace file, helper code, edits, or debugging instructions, while fully staying in character as Andrew."
            )
